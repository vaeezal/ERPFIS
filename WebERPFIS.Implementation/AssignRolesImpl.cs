...
{
    public class AssignRolesImpl : IAssignRoles
    {
        ...
        public IQueryable<UserModel> ShowallRoles(string sortColumn, string sortColumnDir, string Search)
        {
            ...
            var IQueryabletimesheet = (from AssignedRoles in _context.AssignedRoles
                                       ...
                                       join Role in _context.Role on AssignedRoles.AssignToAdmin equals Role.RoleID
                                       select new UserModel
                                       {
                                           Name = registration.Name,
                                           AssignToAdmin = string.IsNullOrEmpty(Role.Rolename) ? "*Not Assigned*" : Role.Rolename.ToUpper(),
            ...
            {
                IQueryabletimesheet = IQueryabletimesheet.Where(x => x.Name.ToUpper().Contains(Search.ToUpper())
                || x.AssignToAdmin.ToUpper().Contains(Search.ToUpper()));
            }
        ...
        public List<RoleModel> ListofRoles()
        {
            ...
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["ERPFISDBEntities"].ToString()))
            {
                ...
                try
                {
                    var result = con.Query<RoleModel>("Usp_GetListofRoles", null, null, true, 0, System.Data.CommandType.StoredProcedure).ToList();
                    result.Insert(0, new RoleModel { Rolename = "----Select----", RoleID = "" });
                    return result;
                }
                catch (Exception)
                {
